...
export default function TrendingTV() {
  ...
  useEffect(() => {
    const fetchTV = async () => {
      try {
        setIsLoading(true);
        const response = await tvService.getTrending(currentPage,"week");
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
      <h1 className="text-3xl font-bold">Series en tendencia esta semana</h1>

      {isLoading ? (
        <div className="text-center py-12">
          <p className="text-gray-400">Cargando series en tendencia...</p>
        </div>
      ) : (
        <>
