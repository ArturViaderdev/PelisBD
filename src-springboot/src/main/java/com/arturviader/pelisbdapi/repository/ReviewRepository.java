...
public interface ReviewRepository extends JpaRepository<Review, Long> {
    Optional<Review> findByUserIdAndTmdbIdAndMediaType(String userId, Long tmdbId, MediaType mediaType);

    @Query("SELECT AVG(r.rating) FROM Review r WHERE r.tmdbId = :tmdbId AND r.mediaType = :mediaType")
    Double findAverageRatingByTmdbIdAndMediaType(@Param("tmdbId") Long tmdbId, @Param("mediaType") MediaType mediaType);

    @Query("SELECT COUNT(r) FROM Review r WHERE r.tmdbId = :tmdbId AND r.mediaType = :mediaType")
    Long countTotalRatingsByTmdbIdAndMediaType(@Param("tmdbId") Long tmdbId, @Param("mediaType") MediaType mediaType);
}
